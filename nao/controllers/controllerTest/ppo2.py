...
class PPO2(ActorCriticRLModel):
    ...
    def __init__(self, policy, env, gamma=0.99, n_steps=128, ent_coef=0.01, learning_rate=2.5e-4, vf_coef=0.5,
                 ...
                 full_tensorboard_log=False, new_tb_run = True):
        ...
        self.tb_initial_timestep = 0
        self.new_tb_run = new_tb_run
    ...
    def learn(self, total_timesteps, num_timesteps, callback=None, seed=None, log_interval=1, tb_log_name="PPO2",
              reset_num_timesteps=True, tb_initial_timestep=0):
        # Transform to callable if needed
        self.learning_rate = get_schedule_fn(self.learning_rate)
        self.cliprange = get_schedule_fn(self.cliprange)
        self.tb_initial_timestep = tb_initial_timestep
        self.num_timesteps = num_timesteps
        print("tb_initial_timestep ", self.tb_initial_timestep)
        new_tb_log = False# self._init_num_timesteps(reset_num_timesteps)
        ...
        with SetVerbosity(self.verbose), TensorboardWriter(self.graph, self.tensorboard_log, tb_log_name) \
            ...
            for update in range(1, nupdates + 1):
                ...
                lr_now = 0.00018#self.learning_rate(self.tb_initial_timestep) #edited from (frac)
                print("Learning rate now = ", lr_now)
                cliprangenow = self.cliprange(1-(self.tb_initial_timestep/100000))#edited from (frac)
                print("Cliprange now = ", cliprangenow)
                ...
                if states is None:  # nonrecurrent version
                    ...
                    for epoch_num in range(self.noptepochs):
                        ...
                        for start in range(0, self.n_batch, batch_size):
                            ...
                            mb_loss_vals.append(self._train_step(lr_now, cliprangenow, *slices, writer=writer,
                                                                 update=timestep+self.tb_initial_timestep))
                    ...
                    for epoch_num in range(self.noptepochs):
                        ...
                        for start in range(0, self.n_envs, envs_per_batch):
                            ...
                            mb_loss_vals.append(self._train_step(lr_now, cliprangenow, *slices, update=timestep + self.tb_initial_timestep,
                ...
                if writer is not None:
                    self.episode_reward = total_episode_reward_logger(self.episode_reward,
                                                                      ...
                                                                      writer, steps = self.num_timesteps + self.tb_initial_timestep)

                if self.verbose >= 1 and (update % log_interval == 0 or update == 1):
                    explained_var = explained_variance(values, returns)
                    logger.logkv("serial_timesteps", update * self.n_steps)
                    logger.logkv("nupdates", update)
                    logger.logkv("total_timesteps", self.num_timesteps + self.tb_initial_timestep)
